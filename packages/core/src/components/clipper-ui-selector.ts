  ...
  render() {
    ...
    return html`
      <div class="relative w-full p-2 text-sm border rounded-md cursor-pointer focus:ring-1 shadow-sm">
        ...
        <div class="${this.isOpen && this.options.length > 0 ? '' : 'hidden'} absolute top-full shadow-sm translate-y-2 left-0 w-full overflow-y-auto border bg-white rounded-md">
          ${this.options.map(
            (option, index) =>
              html`<div class="p-2 cursor-pointer hover:bg-[#f0f0f0]" @click="${() => this.handleOptionClick(option)}">${optionsIcon[index]} ${option.name}</div>`,
          )}
        </div>
      </div>
    `

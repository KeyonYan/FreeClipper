import { LitElement, css, html, unsafeCSS } from 'lit'
import { customElement, property, state } from 'lit/decorators.js'
import tailwindInjectedCss from '../tailwind.out.css?raw'
import type { DatabaseInfo } from '../config'
import CheckIcon from '../assets/check.svg'

@customElement('clipper-ui-selector')
export class ClipperUiSelector extends LitElement {
  @property({ type: Array<DatabaseInfo> }) options: DatabaseInfo[] = []
  @property() selectedOption: DatabaseInfo | null = null
  @property({ type: String }) placeholder = ''
  @property({ type: Function }) onSelect: (...args: any) => any = () => {}
  @state() isOpen = false

  toggleDropdown() {
    this.isOpen = !this.isOpen
  }

  handleOptionClick(option: DatabaseInfo) {
    this.selectedOption = option
    this.isOpen = false
    this.onSelect(option)
  }

  optionIcon(option: DatabaseInfo) {
    if (!option.icon)
      return null
    if (option.icon.type === 'emoji')
      return option.icon.emoji
    return null
  }

  computedIcon(opt: DatabaseInfo) {
    if (!opt.icon)
      return ''
    if (opt.icon.type === 'emoji')
      return opt.icon.emoji
    return ''
  }

  render() {
    const optionsIcon = this.options.map((opt => this.computedIcon(opt)))
    const selectedText = (this.selectedOption && this.selectedOption.name) || this.placeholder
    const selectedIcon = this.selectedOption ? this.computedIcon(this.selectedOption) : ''

    return html`
      <div class="w-full text-sm cursor-pointer ">
        <div class='rounded-md border focus:ring-1 p-2 shadow-sm flex flex-row items-center justify-between' @click="${this.toggleDropdown}">
          <div>${selectedIcon} ${selectedText}</div>
          <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.93179 5.43179C4.75605 5.60753 4.75605 5.89245 4.93179 6.06819C5.10753 6.24392 5.39245 6.24392 5.56819 6.06819L7.49999 4.13638L9.43179 6.06819C9.60753 6.24392 9.89245 6.24392 10.0682 6.06819C10.2439 5.89245 10.2439 5.60753 10.0682 5.43179L7.81819 3.18179C7.73379 3.0974 7.61933 3.04999 7.49999 3.04999C7.38064 3.04999 7.26618 3.0974 7.18179 3.18179L4.93179 5.43179ZM10.0682 9.56819C10.2439 9.39245 10.2439 9.10753 10.0682 8.93179C9.89245 8.75606 9.60753 8.75606 9.43179 8.93179L7.49999 10.8636L5.56819 8.93179C5.39245 8.75606 5.10753 8.75606 4.93179 8.93179C4.75605 9.10753 4.75605 9.39245 4.93179 9.56819L7.18179 11.8182C7.35753 11.9939 7.64245 11.9939 7.81819 11.8182L10.0682 9.56819Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
        </div>
        <div 
          data-ui=${this.isOpen && this.options.length > 0 ? 'show' : 'hidden'} 
          class="transition duration-300
          data-show:translate-y-1 data-show:opacity-100 data-show:h-auto data-show:border data-show:shadow-sm 
          data-hidden:opacity-0 data-hidden:h-0 
          w-full overflow-y-auto 
          bg-white rounded-md">
          ${this.options.map(
            (option, index) =>
              html`
                <div 
                  class="${this.selectedOption?.id === option.id ? 'font-bold' : ''} p-2 m-1 rounded-md flex flex-row justify-between cursor-pointer hover:bg-[#f0f0f0]" 
                  @click="${() => this.handleOptionClick(option)}">
                  <div>${optionsIcon[index]} ${option.name}</div>  
                  ${this.selectedOption?.id === option.id ? html`<img src=${CheckIcon} />` : ''}
                </div>
              `,
          )}
        </div>
      </div>
    `
  }

  static styles = unsafeCSS(tailwindInjectedCss)
}

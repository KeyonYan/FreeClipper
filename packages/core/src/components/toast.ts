        ...
        ${this.toasts.map(
          ({ message, url }) =>
            ...
            <div class="hover:shadow-lg transition-all duration-400 items-center justify-center p-2 flex flex-row gap-2 rounded-lg shadow-md bg-white text-black">
              <div>${message}</div>
              ${url && html`
                <a href=${url} class='inline-flex items-center gap-2 justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-background shadow-sm hover:bg-[#F4F4F5] hover:text-accent-foreground h-9 px-2 py-1'>
